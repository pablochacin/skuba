...
class Skuba:
    ...
    def node_bootstrap(self, cloud_provider=None, timeout=None):
        ...
        self.checker.check_node("master", 0, timeout)
    ...
    def node_join(self, role="worker", nr=0, timeout=None):
        ...
        self.checker.check_node(role, nr, timeout)

    def join_nodes(self, masters=None, workers=None, timeout=None):

	#Override default timeout only if one is defined
        kwargs = {}
        if timeout:
        ...
        for node in range(1, masters):
            self.node_join("master", node, **kwargs)

        for node in range(0, workers, **kwargs):
